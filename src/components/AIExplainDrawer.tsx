...
export default function AIExplainDrawer({
  ...
  // Animate text character by character to simulate streaming
  const animateText = useCallback((fullText: string, msgIndex: number) => {
    let i = 0;
    const step = () => {
      i += Math.ceil(fullText.length / 120); // ~120 frames to display
  ...
  const showSpinner = loading && messages.length <= 1;
    ...
    <>
      ...
      >
        ...
        <div className="flex-1 overflow-y-auto p-4 space-y-4">
          ...
          {messages.map((msg, i) => {
            if (i === 0 && msg.role === "user") return null; // hide initial context
            return (
