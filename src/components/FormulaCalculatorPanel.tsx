...
export default function FormulaCalculatorPanel({ open, onClose }: { open: boolean; onClose: () => void }) {
  ...
  useEffect(() => {
    if (!open) return;
    setLoading(true);
    // Formulas are bundled locally in formulas.json (no DB layer).
              ...
              {selectedFormula && (
                <div className="space-y-4 pt-2">
                  <div className="liquid-glass p-4 space-y-2">
                    ...
                    {selectedFormula.note && <p className="text-xs text-amber-400/80 italic">{selectedFormula.note}</p>}
                  </div>
              ...
              {history.length > 0 && (
                <Collapsible open={showHistory} onOpenChange={setShowHistory}>
                  <CollapsibleTrigger className="flex items-center gap-2 text-xs font-bold text-muted-foreground hover:text-foreground transition w-full">
                    ...
                    <ChevronDown
                      className={`w-3 h-3 ml-auto transition-transform ${showHistory ? "rotate-180" : ""}`}
                    />
                  </CollapsibleTrigger>
                  <CollapsibleContent className="mt-2 space-y-1.5">
                    {history.map((h, i) => (
                      <div
                        key={i}
                        className="flex items-center justify-between bg-muted/50 px-3 py-2 rounded-lg border border-border text-xs"
                      >
                        <span className="text-muted-foreground truncate mr-2">{h.formulaName}</span>
                        <span className="matrix-text font-bold whitespace-nowrap">
                          {h.result} {h.unit}
                        </span>
                      </div>
                    ))}
