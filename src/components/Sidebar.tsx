...
const navItems: { id: ViewId; label: string; icon: React.ReactNode }[] = [
  ...
  { id: "academy", label: "אקדמיה", icon: <SquircleIcon icon={GraduationCap} gradient="rose" /> },
];

export default function Sidebar() {
  const { currentView, navigate, isDark, toggleTheme, progress, data, academyMember, academyOnly } = useApp();
  ...
  const glassButton =
    "w-full flex items-center justify-between p-3 rounded-xl bg-card/40 border border-border/50 text-muted-foreground hover:border-primary/30 hover:text-foreground transition-all duration-200 text-xs font-bold";
  const glassButtonCollapsed =
    "w-full flex items-center justify-center p-3 rounded-xl bg-card/40 border border-border/50 text-muted-foreground hover:border-primary/30 hover:text-foreground transition-all duration-200";

  return (
    <aside
      className={`${isCollapsed ? "w-16" : "w-72"} bg-sidebar border-l border-sidebar-border flex-col shadow-lg z-20 hidden md:flex transition-all duration-300 relative`}
    >
      ...
      {/* Header */}
      <div
        className={`border-b border-sidebar-border flex items-center gap-4 ${isCollapsed ? "p-4 justify-center" : "p-8"}`}
      >
      ...
      <div className={`${isCollapsed ? "p-2" : "p-6"} space-y-3`}>
        {!isCollapsed ? (
          <>
            {isAdmin && (
              <Link to="/admin" className={glassButton}>
                <span>Admin</span>
