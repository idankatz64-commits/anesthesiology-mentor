...
export default function StatsView() {
  ...
  const dailyChanges = useMemo(() => {
    ...
    return {
      ...
      accuracyChange:
        todayAccuracy !== null && yesterdayAccuracy !== null && yesterdayData && yesterdayData.count > 0
          ? Math.round((todayAccuracy - yesterdayAccuracy) * 10) / 10
          : null,
      newUniqueToday,
      newUniqueChange:
        newUniqueYesterday > 0 ? newUniqueToday - newUniqueYesterday : newUniqueToday > 0 ? newUniqueToday : null,
  ...
  const handleImport = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    reader.onload = (ev) => {
      try {
        ...
        alert("שגיאה בקריאת הקובץ. ודא שהקובץ הוא JSON תקני.");
      }
    };
  ...
  const kpiCards = [
    {
      label: "שאלות היום",
      value: dailyChanges.todayQuestions,
      icon: BarChart3,
    ...
    },
    {
      ...
      change: dailyChanges.accuracyChange,
      changeSuffix: "%",
    },
    {
      label: "טעויות פתוחות",
      value: personalStats.uncorrected,
      icon: XCircle,
      color: "text-destructive",
      change: dailyChanges.errorsChange,
      invertColor: true,
    },
    {
      label: "כיסוי מאגר",
      ...
      icon: BookOpen,
      color: "text-primary",
      change: dailyChanges.coverageChangeToday,
      changeSuffix: "%",
    },
  ];
    ...
    <>
      ...
      >
        ...
        <motion.div variants={itemVariants} dir="rtl">
          <button
            ...
            className="w-full flex items-center justify-center gap-2 py-3 px-4 rounded-xl bg-gradient-to-r from-emerald-500/10 to-teal-500/10 border border-emerald-500/25 text-emerald-400 hover:border-emerald-500/50 hover:bg-emerald-500/15 transition font-bold text-sm"
          >
            <TrendingUp className="w-4 h-4" />
            המאמן האישי שלי — ניתוח חולשות + תוכנית שבועית
          </button>
        </motion.div>
        ...
        <motion.div variants={itemVariants} className="grid grid-cols-3 gap-3" dir="rtl">
          ...
          <div className="glass-tile rounded-xl p-2.5 text-center">
            ...
            <div className="text-[9px] text-muted-foreground mt-0.5">
              {data.length > 0 ? Math.round((withExp / data.length) * 100) : 0}% מהמאגר
            </div>
          </div>
          <div className="glass-tile rounded-xl p-2.5 text-center">
            ...
            />
            <div className="text-[9px] text-muted-foreground mt-0.5">
              {data.length > 0 ? Math.round((withoutExp / data.length) * 100) : 0}% מהמאגר
            </div>
          </div>
        </motion.div>
        ...
        <motion.div variants={itemVariants} dir="rtl">
          ...
          <div className="grid grid-cols-3 sm:grid-cols-6 gap-2">
            {[
              {
                value: personalStats.totalAttempts,
                label: "שאלות שבוצעו",
                icon: BarChart3,
                change: dailyChanges.todayQuestions > 0 ? dailyChanges.todayQuestions : null,
                changeSuffix: " היום",
              },
              {
                value: personalStats.uniqueQuestions,
                label: "שאלות ייחודיות",
                icon: BookOpen,
                change: dailyChanges.newUniqueChange,
                changeSuffix: " חדשות",
              },
              {
                value: personalStats.totalErrors,
                label: "טעויות",
                icon: AlertTriangle,
                color: "text-destructive",
              ...
              {
                value: personalStats.corrected,
                label: "מתוקנות",
                click: () => setDrilldownMetric("corrected"),
              ...
              },
              {
                value: personalStats.uncorrected,
                label: "לא תוקנו",
                click: () => setDrilldownMetric("uncorrected"),
                color: "text-primary",
                icon: XCircle,
                iconColor: "text-primary",
              },
              {
                value: personalStats.repeatedErrors,
                label: "טעויות חוזרות",
                click: () => setDrilldownMetric("repeatedErrors"),
                color: "text-destructive",
                icon: Repeat,
                iconColor: "text-destructive",
              },
              ...
              >
                ...
                {"change" in item && item.change !== null && item.change !== undefined && (
                  <div
                    className={`flex items-center justify-center gap-0.5 mt-0.5 text-[9px] font-bold ${
                      item.invertColor
                        ? item.change >= 0
                          ? "text-destructive"
                          : "text-green-500"
        ...
        <motion.div variants={itemVariants} className="grid grid-cols-1 md:grid-cols-2 gap-3">
          <div className="min-h-[300px]">
            <TopicTreemap
              ...
              repeatedErrorsByTopic={repeatedErrorsByTopic}
            />
          </div>
        ...
        <motion.div variants={itemVariants} className="glass-tile rounded-xl p-4" dir="rtl">
          ...
          <div className="flex flex-col sm:flex-row gap-3">
            <button
              ...
              className="px-4 py-2 rounded-lg text-xs font-bold transition flex items-center justify-center gap-2 bg-primary/10 text-primary border border-primary/25 hover:bg-primary/20"
            >
      ...
      {drilldownMetric && (
        <PersonalStatsDrilldown
          ...
          onOpenChange={(open) => {
            if (!open) setDrilldownMetric(null);
          }}
      ...
      <AICoachPanel open={coachOpen} onClose={() => setCoachOpen(false)} questions={data} progress={progress} />
    </>
  );
}
