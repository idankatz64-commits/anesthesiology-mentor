      ...
      try {
        ...
        if (parsed && parsed.history && typeof parsed.history === "object") {
          ...
          if (result.ok) {
            ...
            alert(`חלק מהנתונים לא נשמרו לשרת — נסה שוב.\n(${result.failures.join("; ")})`);
          }
          return;
        }
        ...
        <motion.div variants={itemVariants} className="grid grid-cols-3 gap-3" dir="rtl">
          {[
            { label: "סה״כ שאלות במאגר", value: data.length, color: "text-primary" },
            {
              ...
              value: withExp,
              color: "text-success",
              sub: `${data.length > 0 ? Math.round((withExp / data.length) * 100) : 0}% מהמאגר`,
            },
            {
              label: "ללא הסבר",
              value: withoutExp,
              color: "text-destructive",
              sub: `${data.length > 0 ? Math.round((withoutExp / data.length) * 100) : 0}% מהמאגר`,
            },
          ].map((item) => (
            <StatCard
              key={item.label}
