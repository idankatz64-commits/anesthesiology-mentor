...
export default function FlashcardView() {
  ...
  const allTopics = useMemo(() => {
    ...
    data.forEach((q) => {
      if (q[KEYS.TOPIC]) topics.add(q[KEYS.TOPIC]);
    });
  ...
  const handleConfidence = async (level: ConfidenceLevel) => {
    ...
    try {
      ...
      // A failed SRS write must not strand the user mid-deck — log and continue.
      console.error("Flashcard SRS update failed:", err);
    }
  ...
  // ─── SETUP SCREEN ───
  if (phase === "setup") {
    const canStart = mode === "topics" ? selectedTopics.size > 0 : mode === "mistakes" ? mistakeCount > 0 : true;
        ...
        <div className="space-y-4 mb-8">
          ...
          >
            <div className="flex items-start gap-4">
              <div
                className={`w-10 h-10 rounded-xl flex items-center justify-center shrink-0 ${mode === "topics" ? "bg-primary text-primary-foreground" : "bg-muted text-muted-foreground"}`}
              >
              ...
              <div className="flex-1">
                ...
                <p className="text-sm text-muted-foreground mt-0.5">
                  בחר נושא אחד או יותר ליצירת חפיסת כרטיסיות ממוקדת.
                </p>
              </div>
          ...
          >
            <div className="flex items-start gap-4">
              <div
                className={`w-10 h-10 rounded-xl flex items-center justify-center shrink-0 ${mode === "mistakes" ? "bg-destructive text-destructive-foreground" : "bg-muted text-muted-foreground"}`}
              >
          ...
          >
            <div className="flex items-start gap-4">
              <div
                className={`w-10 h-10 rounded-xl flex items-center justify-center shrink-0 ${mode === "smart" ? "bg-primary text-primary-foreground" : "bg-muted text-muted-foreground"}`}
              >
              ...
              <div className="flex-1">
                <h3 className="font-bold text-foreground">
                  למידה חכמה{" "}
                  <span className="text-xs bg-primary/10 text-primary px-2 py-0.5 rounded-full mr-2">מומלץ</span>
                </h3>
                <p className="text-sm text-muted-foreground mt-0.5">
                  האלגוריתם בוחר כרטיסיות על סמך חזרה מרווחת ורמת ביטחון.
                </p>
              </div>
  ...
  return (
    <div className="fade-in max-w-2xl mx-auto py-8">
      ...
      <div style={{ perspective: "1000px" }}>
        ...
        >
          {/* Front */}
          <div className="w-full rounded-2xl deep-tile p-8 md:p-12 shadow-lg" style={{ backfaceVisibility: "hidden" }}>
          ...
          >
            ...
            {current[KEYS.MILLER] && current[KEYS.MILLER].trim() && (
              <div className="bg-muted/50 rounded-lg px-4 py-2 mb-6 text-sm text-muted-foreground">
                📖 Miller: {current[KEYS.MILLER]}
                {current[KEYS.CHAPTER] ? `, Chapter ${current[KEYS.CHAPTER]}` : ""}
              </div>
            )}
