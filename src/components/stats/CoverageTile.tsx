...
export default function CoverageTile({ coverage, chapters }: Props) {
  return (
    <AnimatedStatsTile
      ...
      expanded={
        <div>
          <h3 className="text-lg font-bold text-foreground mb-2">כיסוי לפי פרק</h3>
          <p className="text-xs text-muted-foreground mb-4">כל ריבוע = פרק. צבע לפי אחוז השלמה.</p>
          <div className="grid grid-cols-10 gap-1.5">
            {chapters.map(ch => (
              <div
                key={ch.chapter}
                className={`aspect-square rounded-md flex items-center justify-center text-[9px] font-bold text-foreground/70 ${getHeatColor(ch.pct)}`}
          ...
          <div className="flex items-center gap-4 mt-4 text-[10px] text-muted-foreground">
            <span className="flex items-center gap-1"><span className="w-3 h-3 rounded bg-muted/30 inline-block" /> 0%</span>
